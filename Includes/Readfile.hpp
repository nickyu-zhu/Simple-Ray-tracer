    ...
    if (in.is_open()) {
        ...
        while (in) {
            if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) { // Ruled out comment and blank lines 
                ...
                s >> cmd;
                int i;
                GLfloat values[10]; // max num of params = 10 (camera)
                ...
                // directional & point command
                    ...
                    if (numLights == maxNumLights) { // No more lights 
                        ...
                        if (validinput) {
                            for (int i = 0; i < 3; i++) {
                                lightPos[4 * numLights + i] = values[i];
                                lightColor[4 * numLights + i] = values[3 + i];
                            }
                            if (cmd == "directional") lightPos[4 * numLights + 3] = 0;
                            else if (cmd == "point") lightPos[4 * numLights + 3] = 1;
                            lightColor[4 * numLights + 3] = 1;
                            numLights++;
                        }
                    }
                }
                ...
                // material settings 
                    ...
                    if (validinput) {
                        ...
                        ambient[3] = 1; // set objects as opaque by default
                    }
                }
                else if (cmd == "diffuse") {
                    validinput = readvals(s, 3, values);
